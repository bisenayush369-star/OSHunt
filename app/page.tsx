...
const ISSUES = [
  { n: "01", title: "useSearchParams causes hydration mismatch in layouts", repo: "vercel/next.js", pill: "pill-med", diff: "medium", stars: "122k" },
  { n: "02", title: "MongoDB _count returns undefined for empty relations", repo: "prisma/prisma", pill: "pill-easy", diff: "easy", stars: "38k" },
  { n: "03", title: "res.json() ignores replacer function in options", repo: "expressjs/express", pill: "pill-easy", diff: "easy", stars: "63k" },
  { n: "04", title: "useEffect cleanup skipped on fast refresh cycle", repo: "facebook/react", pill: "pill-hard", diff: "hard", stars: "224k" },
  { n: "05", title: "populate() with lean() drops virtuals on nested docs", repo: "mongoose/mongoose", pill: "pill-med", diff: "medium", stars: "26k" },
];
...
export default function Home() {
  return (
    <>
      ...
      {/* 🚀 Your perfect working navbar component handles ALL the logic now! */}
      <HomeNav />
      ...
      <div className="proof">
        ...
        <div className="proof-logos">
          {["facebook/react", "vercel/next.js", "expressjs/express", "prisma/prisma", "mongoose/mongoose"].map((r) => (
            <span key={r} className="proof-logo">{r}</span>
          ))}
        </div>
      </div>

      <section id="how-it-works" style={{ padding: "5rem 2rem", maxWidth: 780, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 0, marginTop: "3rem" }}>
          {[
            { n: "01", title: "Pick your stack", desc: "Select your language and difficulty. OSHunt filters thousands of real GitHub issues down to only what you can actually fix." },
            { n: "02", title: "AI reads the bug for you", desc: "Gemini reads the issue and surrounding code. You get a plain-English breakdown — what's broken, how hard it is, where to start." },
            { n: "03", title: "Understand the repo with GitLense", desc: "Paste the repo URL. GitLense explains the entire codebase architecture before you touch a single file." },
            { n: "04", title: "Fix it and ship it", desc: "Open the issue on GitHub, apply your fix, submit a PR. Your contribution gets tracked on your profile." },
          ].map((step, i, arr) => (
            <div key={step.n} style={{ display: "flex", gap: 28, paddingBottom: i < arr.length - 1 ? "2.5rem" : 0, marginBottom: i < arr.length - 1 ? "2.5rem" : 0, borderBottom: i < arr.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
              <div style={{ fontFamily: "monospace", fontSize: 11, color: "#a8ff3e", paddingTop: 4, flexShrink: 0, letterSpacing: 1 }}>{step.n}</div>
              <div>
                <p style={{ fontSize: 16, fontWeight: 600, letterSpacing: -0.4, marginBottom: 8 }}>{step.title}</p>
                <p style={{ fontSize: 14, color: "#666", lineHeight: 1.7, fontWeight: 300 }}>{step.desc}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="preview">
        <div className="preview-window">
          <div className="preview-bar">
            ...
            <span className="preview-title-txt">oshunt — matched to your stack</span>
          </div>
          {ISSUES.map((i) => (
