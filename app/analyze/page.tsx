...
interface AnalysisResult {
  ...
  /** Everything below is new and optional — older responses (or a raw-text
   *  fallback when the model's JSON didn't parse) simply won't have it, and
   *  every place that reads these fields is written to degrade gracefully. */
...
const GitHubIcon = () => (
  <Image
    ...
    width={13}
    height={13}
    style={{ filter: "invert(62%) sepia(46%) saturate(648%) hue-rotate(72deg) brightness(89%)" }}
  />
)
...
const StarIcon = () => <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor" stroke="none"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"></path></svg>
      ...
      <div className="relative mx-auto w-full max-w-[840px] px-4 pb-16 pt-14 sm:px-6 sm:pb-20 sm:pt-16 lg:px-8 lg:pt-20">
        ...
        {(result || rawResult) && !loading && (
          <div className="animate-[fade-up_0.4s_ease_forwards] motion-reduce:animate-none">
            ...
            {/* Repository metadata strip — only renders what's actually present,
                so a raw-text fallback (model JSON didn't parse) degrades cleanly. */}
            {(result?.metadata || result?.framework || result?.repositoryType) && (
              <div className="mb-5 flex flex-wrap items-center gap-1.5">
                {typeof result?.metadata?.stars === "number" && (
                  <Badge variant="outline" className="gap-1 rounded-md border-[#1a1a1a] bg-[#0a0a0a] px-2 py-1 text-[11px] font-normal text-[#999]">
                    <StarIcon /> {result.metadata.stars.toLocaleString()}
                  </Badge>
                )}
                {result?.framework && (
                  <Badge variant="outline" className="rounded-md border-[#a8ff3e]/15 bg-[#a8ff3e]/[0.06] px-2 py-1 text-[11px] font-normal text-[#a8ff3e]">
                    {result.framework}
                  </Badge>
                )}
                {result?.repositoryType && (
                  <Badge variant="outline" className="rounded-md border-[#1a1a1a] bg-[#0a0a0a] px-2 py-1 text-[11px] font-normal capitalize text-[#999]">
                    {result.repositoryType === "monorepo" ? "Monorepo" : "Single package"}
                  </Badge>
                )}
                {result?.metadata?.license && (
                  <Badge variant="outline" className="rounded-md border-[#1a1a1a] bg-[#0a0a0a] px-2 py-1 text-[11px] font-normal text-[#999]">
                    {result.metadata.license}
                  </Badge>
                )}
                {result?.metadata?.topics && result.metadata.topics.length > 0 && result.metadata.topics.slice(0, 4).map((topic) => (
                  <Badge key={topic} variant="outline" className="rounded-md border-[#1a1a1a] bg-[#0a0a0a] px-2 py-1 text-[11px] font-normal text-[#777]">
                    #{topic}
                  </Badge>
                ))}
              </div>
            ...
            {result ? (
              <div className="mb-3 flex flex-col">
                    ...
                    {result.techStack.length > 0 ? (
                      <div className="flex flex-wrap gap-1.5">
                        {result.techStack.map((tech, i) => (
                          <span
                            key={i}
                            className="rounded-md border border-[#1a1a1a] bg-black px-2.5 py-1 text-[12.5px] text-[#e0e0e0] transition-all duration-200 hover:-translate-y-0.5 hover:border-[#a8ff3e]/25 hover:text-[#a8ff3e]"
                          >{tech}</span>
                        ))}
                      </div>
                    ) : (
                ...
                {/* Architecture — new, only present when the backend provided it */}
                {result.architecture && (
                  <FadeInView className="border-t border-[#141414] py-6">
                    <ResultLabel icon={<LayersIcon />}>Architecture</ResultLabel>
                    <p className="max-w-[64ch] text-[15px] leading-[1.7] text-[#ccc]"><Reveal text={result.architecture} /></p>
                  </FadeInView>
                )}
